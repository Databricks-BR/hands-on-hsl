...
# MAGIC
# MAGIC > **Pré-requisito:** Este notebook assume que as tabelas `healthcare_lakehouse.bronze.hospitais`,
# MAGIC > `healthcare_lakehouse.bronze.pacientes` e `healthcare_lakehouse.bronze.consultas` já existem
...
# MAGIC CREATE OR REPLACE TABLE healthcare_lakehouse.bronze.consultas_demo AS
# MAGIC WITH pacientes_amostra AS (
# MAGIC     SELECT id_paciente, row_number() OVER (ORDER BY id_paciente) AS rn
# MAGIC     FROM healthcare_lakehouse.bronze.pacientes
...
# MAGIC     FROM healthcare_lakehouse.bronze.hospitais
# MAGIC )
# MAGIC SELECT
...
# MAGIC CREATE OR REPLACE VIEW healthcare_lakehouse.bronze.consultas AS
# MAGIC SELECT * FROM healthcare_lakehouse.bronze.consultas_demo;
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE VIEW healthcare_lakehouse.silver.v_consultas_enriquecidas_ia
...
# MAGIC FROM healthcare_lakehouse.bronze.consultas c
# MAGIC WHERE c.sintomas_relatados IS NOT NULL;
