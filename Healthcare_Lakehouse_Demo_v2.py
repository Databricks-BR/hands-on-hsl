...
# MAGIC
# MAGIC ### Por que parametrizar?
# MAGIC Em ambientes compartilhados (workshops, multi-tenant, equipes) cada participante precisa do
# MAGIC **próprio catálogo isolado** para não sobrescrever dados de outros. A célula abaixo cria um
# MAGIC **widget** que pergunta seu nome/identificador e usa como sufixo do catálogo.
# MAGIC
# MAGIC > **Exemplo:** com `sufixo = leandro`, o catálogo criado será **`healthcare_lakehouse_leandro`**.
# MAGIC
# MAGIC ### Como usar
# MAGIC 1. Execute a célula abaixo (ela cria o widget)
# MAGIC 2. No topo do notebook, edite o campo **`sufixo`** com seu nome (sem espaços/acentos)
# MAGIC 3. Continue executando o notebook normalmente — todas as referências usam `${catalog_name}` automaticamente

# COMMAND ----------

# DBTITLE 1,Criar widget de sufixo
# MAGIC %sql
# MAGIC -- Edite o valor padrão se quiser, ou ajuste no widget no topo do notebook
...
# MAGIC %sql
# MAGIC -- Concatena o prefixo fixo com o sufixo do widget e define como variável de sessão
# MAGIC DECLARE OR REPLACE VARIABLE catalog_name STRING;
# MAGIC SET VAR catalog_name = CONCAT('healthcare_lakehouse_', :sufixo);
# MAGIC
# MAGIC SELECT catalog_name AS catalogo_que_sera_criado;
...
# MAGIC 1. Criar (idempotente) o catálogo `healthcare_lakehouse_<sufixo>` (ex: `healthcare_lakehouse_leandro`)
...
# MAGIC - **`IDENTIFIER(:var)`**: permite usar nome de catálogo dinamicamente em DDL/DML
# MAGIC
...
# MAGIC
# MAGIC **Resultado esperado:** Tabela `healthcare_lakehouse_${sufixo}.bronze.hospitais` com **140 registros** × **12 colunas**.
...
# MAGIC
# MAGIC > **Resultado esperado:** Tabela `healthcare_lakehouse_${sufixo}.bronze.pacientes` com **1500 registros** × **12 colunas** — 500 da população geral + 1000 com perfil mais jovem (target: 70% < 35 anos).
...
# MAGIC
# MAGIC 1. **Criar** o Volume `healthcare_lakehouse_${sufixo}.bronze.landing_zone`
...
# MAGIC CREATE VOLUME IF NOT EXISTS healthcare_lakehouse_${sufixo}.bronze.landing_zone
...
# MAGIC INSERT OVERWRITE DIRECTORY '/Volumes/healthcare_lakehouse_${sufixo}/bronze/landing_zone/consultas'
# MAGIC   USING CSV
...
# MAGIC JOIN healthcare_lakehouse_${sufixo}.bronze.hospitais h ON h.id_hospital = cp.id_hospital
...
# MAGIC LIST '/Volumes/healthcare_lakehouse_${sufixo}/bronze/landing_zone/consultas/';
...
# MAGIC   '/Volumes/healthcare_lakehouse_${sufixo}/bronze/landing_zone/consultas/',
# MAGIC   format => 'csv',
# MAGIC   header => true
# MAGIC )
...
# MAGIC %sql
# MAGIC INSERT OVERWRITE DIRECTORY '/Volumes/healthcare_lakehouse_${sufixo}/bronze/landing_zone/avaliacoes'
# MAGIC   USING CSV
# MAGIC   OPTIONS ('header' 'true')
# MAGIC WITH
...
# MAGIC SELECT *
# MAGIC FROM read_files(
# MAGIC   '/Volumes/healthcare_lakehouse_${sufixo}/bronze/landing_zone/avaliacoes/',
# MAGIC   format => 'csv', header => true
# MAGIC )
# MAGIC ORDER BY id_consulta
# MAGIC LIMIT 10;
...
# MAGIC %sql
# MAGIC LIST '/Volumes/healthcare_lakehouse_${sufixo}/bronze/landing_zone/';
...
# MAGIC
# MAGIC ```
# MAGIC healthcare_lakehouse_${sufixo}
# MAGIC ├── bronze
...
# MAGIC 2. Adicione a fonte: **read_files** apontando para `/Volumes/healthcare_lakehouse_${sufixo}/bronze/landing_zone/consultas/`
